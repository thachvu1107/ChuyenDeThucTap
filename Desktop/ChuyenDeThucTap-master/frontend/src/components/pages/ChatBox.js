  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const patterns = [
        ...
        { key: "tim", action: "searchProducts" }, // Add specific pattern
        { key: "tim xe", action: "searchProducts" },
        { key: "tim phu kien", action: "searchProducts" },
        { key: "tim xe dap", action: "searchProducts" },
      ...
      if (matchedPattern) {
        switch (matchedPattern.action) {
          case "greeting": {
            const reply = `Chào bạn! 😊 Tôi là THEGIOIXEDAP#STORE Chatbot, hỗ trợ bạn mua sắm dễ dàng! Bạn có thể:
- Tìm sản phẩm: "tìm xe đạp"
...
Hãy hỏi bất kỳ câu gì, tôi sẽ giúp ngay!`;
          ...
          case "searchProducts": {
              ...
              if (!searchInput) {
                setMessages((prev) => [
                  ...prev,
                  {
                    role: "bot",
                    content:
                      'Vui lòng cung cấp từ khóa tìm kiếm. Ví dụ: "tìm xe", "xem xe đạp", hoặc "tìm phụ kiện".',
                  },
                ]);
                return;
              }
            ...
            if (products.length === 0) {
              setMessages((prev) => [
                ...
                {
                  role: "bot",
                  content: `Không tìm thấy sản phẩm nào với từ khóa "${searchInput}". Hãy thử từ khóa khác như "xe đạp"!`,
                },
              ]);
              return;
            }
  ...
  return (
    <div className="chatbot-container">
      <h2 className="chatbot-title">THEGIOIXEDAP#STORE Chatbot</h2>
        ...
        {messages.map((msg, index) => (
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: `<strong>${
                msg.role === "user" ? "Bạn: " : "THEGIOIXEDAP#STORE Bot: "
              }</strong>${msg.content}`,
            }}
          />
        ))}
